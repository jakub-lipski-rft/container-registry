...
For more information about this feature please see the [dedicated docs](https://gitlab.com/gitlab-org/container-registry/-/blob/master/docs-gitlab/migration-proxy.md).

| Parameter | Required | Description                                           |
|-----------|----------|-------------------------------------------------------|
| `enabled` | no  | If the proxy mode is enabled (boolean). Defaults to `false`.           |
| `url`         | yes  | The URL of the target registry instance.           |
| `include` | no  | A list of regular expressions used to filter repositories by name. Only repositories that match one of these regular expressions are proxied.           |
| `exclude` | no  | A list of regular expressions used to filter repositories by name. Only repositories that do not match one of these regular expressions are proxied. Repositories included through `include` can be excluded by `exclude`.          |

Please note that both instances should be configured with the same HTTP secret (`http.secret`). Furthermore, the new instance HTTP host (`http.host`) must be configured with the host of the existing instance. This is required to ensure that `Location` headers generated by the new (private) instance contain the host of the existing (public) instance. 
