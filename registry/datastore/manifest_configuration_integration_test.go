...
func reloadManifestConfigurationFixtures(tb testing.TB) {
	testutil.ReloadFixtures(tb, suite.db, suite.basePath, testutil.ManifestConfigurationsTable)
}

func unloadManifestConfigurationFixtures(tb testing.TB) {
	require.NoError(tb, testutil.TruncateTables(suite.db, testutil.ManifestConfigurationsTable))
}
