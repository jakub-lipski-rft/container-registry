...
func TestDoWalk(t *testing.T) {
	...
	standardDriver, err := s3DriverConstructor(rootDir, s3.StorageClassStandard)
	if err != nil {
		t.Fatalf("unable to create driver with standard storage: %v", err)
	}

	ctx := context.Background()
	wantedFiles := 10
	for i := 0; i < wantedFiles; i++ {
		filename := "/walk/subdir/file" + strconv.Itoa(i)
		contents := []byte("contents")
		err = standardDriver.PutContent(ctx, filename, contents)
		if err != nil {
			t.Fatalf("unable to create content: %v", err)
		}
	}
	...
	var actualFiles int
	standardDriverUnwrapped := standardDriver.Base.StorageDriver.(*driver)
	err = standardDriverUnwrapped.doWalk(ctx, &objCount, rootDir, "walk", func(fInfo storagedriver.FileInfo) error {
		if !fInfo.IsDir() {
			actualFiles++
		}
		return nil
	...
	if err != nil {
		t.Fatalf("doWalk: %v", err)
	}

	if actualFiles != wantedFiles {
		t.Fatalf("expected %d files, got %d", wantedFiles, actualFiles)
	}

	// cant actually verify deletion because read-after-delete is inconsistent, but can ensure no errors
	err = standardDriver.Delete(ctx, "/walk")
	if err != nil {
		t.Fatalf("unable to delete doWalk test files: %v", err)
	}
}
