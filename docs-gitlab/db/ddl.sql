...
CREATE TABLE manifests (
    ...
    CONSTRAINT unique_manifests_configuration_blob_digest_and_repo_id_and_id UNIQUE (configuration_blob_digest, repository_id, id)
)
PARTITION BY HASH (repository_id);
...
CREATE TABLE layers (
    ...
    CONSTRAINT unique_layers_digest_and_repository_id_and_id UNIQUE (digest, repository_id, id)
)
PARTITION BY HASH (repository_id);
...
CREATE INDEX index_gc_blobs_layers_on_digest_and_repository_id_and_layer_id ON gc_blobs_layers USING btree (digest, repository_id, layer_id);
...
CREATE INDEX index_gc_blobs_configurations_on_digest_and_repo_id_and_man_id ON gc_blobs_configurations USING btree (digest, repository_id, manifest_id);
