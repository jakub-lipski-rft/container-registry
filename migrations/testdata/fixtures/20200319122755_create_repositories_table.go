	...
	m := &migrations.Migration{Migration: &migrate.Migration{
		...
		Up: []string{
			`CREATE TABLE IF NOT EXISTS repositories_test (
                ...
                CONSTRAINT uq_repositories_test_path UNIQUE (path),
                CONSTRAINT ck_repositories_test_name_length CHECK ((char_length(name) <= 255)),
                CONSTRAINT ck_repositories_test_path_length CHECK ((char_length(path) <= 255))
            )`,
			"CREATE INDEX IF NOT EXISTS ix_repositories_parent_id ON repositories_test (parent_id)",
		},
		Down: []string{
			"DROP INDEX IF EXISTS ix_repositories_test_parent_id CASCADE",
