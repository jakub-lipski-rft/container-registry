	...
	m := &migrations.Migration{Migration: &migrate.Migration{
		...
		Up: []string{
			`CREATE TABLE IF NOT EXISTS tags_test (
                ...
                CONSTRAINT uq_tags_test_name_repository_id UNIQUE (name, repository_id),
                CONSTRAINT ck_tags_test_name_length CHECK ((char_length(name) <= 255))
            )`,
			"CREATE INDEX IF NOT EXISTS ix_tags_test_repository_id ON tags_test (repository_id)",
			"CREATE INDEX IF NOT EXISTS ix_tags_test_manifest_id ON tags_test (manifest_id)",
		},
		Down: []string{
			"DROP INDEX IF EXISTS ix_tags_test_manifest_id CASCADE",
			"DROP INDEX IF EXISTS ix_tags_test_repository_id CASCADE",
